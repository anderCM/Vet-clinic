...
SELECT COUNT(id) total_animals
FROM animals;
/* How many animals have never tried to escape? */
SELECT COUNT(id) animals_no_escape
FROM animals
WHERE escape_attempts = 0;
/* What is the average weight of animals? */
SELECT ROUND(AVG(weight_kg), 2) average_weight
FROM animals;
...
SELECT species,
	ROUND(AVG(escape_attempts), 2) average_escape
FROM animals
...
WHERE own.fullname = 'Melody Pond';
/* List of all animals that are pokemon */
SELECT anm.*
FROM animals anm
	JOIN species spc ON spc.id = anm.species_id
WHERE spc.name = 'Pokemon';
/* List all owners and their animals, remember to include those that don't own any animal */
SELECT anm.name animal,
	own.fullname owner_name
FROM animals anm
	FULL JOIN owners own ON own.id = anm.owner_id;
/* How many animals are there per species */
SELECT spc.name specie,
	COUNT(anm.id) quantity
FROM animals anm
	JOIN species spc ON spc.id = anm.species_id
GROUP BY spc.name;
/* List all Digimon owned by Jennifer Orwell */
SELECT anm.name animal_name
FROM animals anm
	JOIN owners own ON own.id = anm.owner_id
	JOIN species spc ON spc.id = anm.species_id
...
SELECT anm.name animal_name,
	COUNT(vis.animal_id) visits_quantity
FROM animals anm
	JOIN visits vis ON vis.animal_id = anm.id
...
SELECT COUNT(anm.id) visits_no_specialized
FROM animals anm
	JOIN visits vis ON vis.animal_id = anm.id
	JOIN vets vts ON vts.id = vis.vet_id
...
SELECT spc.name speciality_considered,
	COUNT(anm.id) quantity
FROM animals anm
	JOIN visits vis ON vis.animal_id = anm.id
	JOIN vets vts ON vts.id = vis.vet_id
...
FROM visits
where animal_id = 4;
CREATE INDEX anm_id_index ON visits(animal_id);
EXPLAIN ANALYZE
SELECT *
FROM visits
where vet_id = 2;
